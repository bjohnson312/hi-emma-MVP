...
import { Sun, MessageCircle, CheckCircle2, Circle, Clock, Sparkles } from "lucide-react";
...
export default function MorningRoutineView({ userId }: MorningRoutineViewProps) {
  ...
  const [selectedRoutine, setSelectedRoutine] = useState<MorningRoutinePreference | null>(null);
  ...
  async function loadData() {
    try {
      ...
      setSelectedRoutine(preferenceRes.preference);
      ...
      if (todayRes.completion?.activities_completed) {
        ...
        setCompletedActivities(Array.isArray(activities) ? activities : []);
      }
  ...
  async function selectTemplate(template: RoutineTemplate) {
    ...
    try {
      const newRoutine = await backend.morning.createRoutinePreference({
        user_id: userId,
  ...
  async function toggleActivity(activityId: string) {
    ...
    const allDone = newCompleted.length === selectedRoutine.activities.length;
  ...
  if (showTemplates || !selectedRoutine) {
    return (
      <div className="space-y-6">
        <div className="bg-white/95 backdrop-blur-md rounded-3xl p-6 shadow-xl border border-white/40">
          <h2 className="text-2xl font-bold text-[#323e48] mb-2">Choose Your Morning Routine</h2>
          <p className="text-[#323e48]/70 mb-6">Pick a routine that fits your lifestyle</p>
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-[#4e8f71]">{selectedRoutine.routine_name}</p>
            </div>
          </div>
        ...
        <div className="bg-gradient-to-r from-[#4e8f71]/10 to-[#364d89]/10 rounded-2xl p-6 border border-[#4e8f71]/20">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-sm text-[#323e48]/60">
              {completedActivities.length}/{selectedRoutine.activities.length}
            </span>
          </div>
          ...
          {completedActivities.length === selectedRoutine.activities.length && selectedRoutine.activities.length > 0 && (
